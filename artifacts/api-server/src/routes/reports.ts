...
import { formatFactoryDateTime, getFactoryDate, getFactoryNow } from "../lib/factoryTime";
import { requireRoles } from "../lib/authz";

const router: IRouter = Router();

const toNumber = (value: string | null) => (value === null ? null : Number(value));
const isCurrentReportAlias = (value: string) => value === "current" || value === "latest";
...
router.get("/reports/daily/:productionDate", requireRoles("MANAGER", "ADMIN"), async (req, res, next) => {
  try {
    ...
    const productionDate = isCurrentReportAlias(String(req.params.productionDate))
      ? getFactoryDate(undefined, factory.timezone)
      : String(req.params.productionDate);
    const day = (await db
      .select()
      .from(productionDays)
      .where(
    ...
    const report = {
      ...
      generatedAt: getFactoryNow().toISOString(),
      executiveSummary:
...
router.get("/reports/daily/:productionDate/pdf", requireRoles("MANAGER", "ADMIN"), async (req, res, next) => {
  try {
    ...
    const productionDate = isCurrentReportAlias(String(req.params.productionDate))
      ? getFactoryDate(undefined, factory.timezone)
      : String(req.params.productionDate);
    const day = await db.select().from(productionDays).where(and(
      eq(productionDays.factoryId, factoryId),
      eq(productionDays.productionDate, productionDate),
    )).limit(1);
    if (!day[0]) {
    ...
    document.moveDown(0.25).fontSize(11).fillColor("#4b5563").text(factory.name);
    document.fontSize(14).fillColor("#111827").text(`Daily management report — ${productionDate}`);
    document.moveDown(0.25).fontSize(10).fillColor("#374151").text(`Reporting Date: ${productionDate}`);
    document.moveDown(0.25).fontSize(9).fillColor("#6b7280").text(
      `Season 2025–26 · report version phase-1.1 · Generated: ${formatFactoryDateTime(new Date(), factory.timezone)}`,
    );
...
router.get("/reports/daily/:productionDate/lineage", requireRoles("MANAGER", "ADMIN"), async (req, res, next) => {
  try {
    ...
    const [factory] = factoryId
      ? await db.select({ timezone: factories.timezone }).from(factories).where(eq(factories.id, factoryId)).limit(1)
      : [];
    const productionDate = isCurrentReportAlias(String(req.params.productionDate))
      ? getFactoryDate(undefined, factory?.timezone)
      : String(req.params.productionDate);
